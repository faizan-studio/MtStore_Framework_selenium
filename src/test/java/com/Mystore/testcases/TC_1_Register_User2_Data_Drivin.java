...
public class TC_1_Register_User2_Data_Drivin extends BaseTestcases {
	@Test(enabled = false)
	...
	public void loginUser(String Useremail,String UserPw , String Expname,String ExpDel) throws IOException {
		Exercise_page exp = new Exercise_page(dr) ;
		
		exp.signup();
		
		Login_page lp = new Login_page(dr);
		lp.Enteredemailadd(Useremail);
		lp.Enteredpass(UserPw);
		lp.Loginbutton();
	...
	public String[][]  logindataprovider(){
		...
		String filename = System.getProperty("user.dir") + "\\Test Data\\MyStoreV1_Test_Data.xlsx";
		int ttlrows = ReadExcelFile.getrowcount(filename, "Verify_Login");
		int ttlcoloums =ReadExcelFile.gecoloumcount(filename, "Verify_Login");
		
		String data[][] = new String [ttlrows -1][ttlcoloums];
		
		for(int i =1;i<ttlrows; i++) {
			for(int j =0 ;j<ttlcoloums;j++)
			{
				data [i-1][j] =ReadExcelFile.getcellvalue(filename, "Verify_Login", i, j);
			}
		}
		return data;
	}
